...
module Cucumber
  module Formatter
	...
	passed_values = []
	passed_keys = []
	my_hash = Hash[*@exceptions_hash]
	my_hash.values.each {|x|
	  passed_values.include?(x.to_s) || passed_values << x.to_s && print_exception(x, :failed, 6)
	  Hash[*my_hash.select{|k,v| v.to_s == x.to_s}.flatten].keys.each {|y|
	     if (!passed_keys.include?(y.to_s))
		passed_keys << y.to_s
		before_table_row(y)
		table_cell_value(y, :failed)
		@io.puts
	     end
	  }
        }
      ...
      def after_table_row(table_row)
        ...
        if table_row.exception && !@exceptions.include?(table_row.exception)
          ...
          @exceptions_hash << table_row.exception
        end
      end
	...
	cell_text2 = cell_text.gsub("hm","CLOUD")
        padded = cell_text2 + (' ' * (width - cell_text.jlength))
        prefix = cell_prefix(status)
        @io.print(' ' + format_string("#{prefix}#{padded}", status) + ::Term::ANSIColor.reset(" |"))
        @io.flush
	if status == :failed
	   @exceptions_hash << value
	end
      end
